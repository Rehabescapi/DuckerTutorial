...
import {
  ...
  header, text, likeReplyContainer, icon, likeIcon, author,
} from './styles.css'
...
export default function Duck (props) {
  const starIcon = props.isLiked === true ? likeIcon : icon
  const starFn = props.isLiked === true
    ? props.handleDeleteLike : props.addAndHandleLike
  return (
    <div
      ...
      <div className={contentContainer}>
        <div className={header}>
          <div onClick={props.goToProfile} className={author}>
            {props.duck.get('name')}</div>
          <div>{formatTimestamp(props.duck.get('timestamp'))}
          </div>
        </div>
